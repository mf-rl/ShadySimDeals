...
- `eligible_household_member_ids()` excludes newborns before picker-row construction. Toddler-through-elder household sales use `RabbitHoleService.put_sims_in_shared_rabbithole` and `set_rabbit_hole_expiration_callback` with `rabbit_hole.multi_sim_rabbit_hole.TwoSimRabbitHole`. Infants use native pickup `271032` or handoff continuation `269721`; after ownership is verified, only the seller enters the private 90-minute managed `RabbitHole` and the infant remains attached without a separate routing interaction.
...
- Rabbit-hole tuning uses resource type `0xB16AD2FA`, generic rabbit-hole animation factory `23834`, household rabbit-hole IDs `0xEAA21FFB1081E005`-`007`, unborn rabbit-hole IDs `0xEAA21FFB1081E00B`-`010`, and rabbit-hole affordance IDs `0xEAA21FFB1081E008`-`00A` and `011`-`013`. Dormant newborn pickup affordance `0xEAA21FFB1081E025` remains packaged but unreachable.
...
5. Confirm the picker includes on-lot infant-through-elder household members, excludes newborns and the active Sim, and excludes household members at work, school, or otherwise off-lot. Repeat the off-lot check for the unborn picker.
