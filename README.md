...
Eligible targets are on-lot household members from newborn/baby through elder. Newborns use their household `SimInfo` because they do not expose a normal instantiated Sim. The active Sim, pets, Sims at work or school, other off-lot Sims, already-sold Sims, and Sims involved in another transaction are excluded. The unborn picker applies the same on-lot requirement.

The household-member offer starts with the target's age value. If the target is pregnant, the configured unborn value and multiple-offspring multiplier are added to the offer; the pregnancy remains with the transferred Sim. After confirmation, sellers and targets from toddler through elder enter one shared rabbit hole for 75 Sim minutes when selling an elder, 90 for toddler through child, or 120 for teen through adult. For an infant, the seller first uses the game's native pickup interaction; another carrier hands the infant to the seller. For a newborn, the current carrier completes the game's visible natural put-down. After that exact Held Actions interaction fully leaves the carrier's interaction state, the seller reserves the newborn and starts a private invisible pickup interaction whose only outcome is EA's native Cuddle under persistent Held Actions. After the private interaction fully leaves the seller's interaction state, one settlement tick verifies targeted Held Actions and seller carry ownership before proceeding. Only the seller is then registered in a 90-minute rabbit hole while the attached target travels and disappears with them. Natural expiration returns only the seller, transfers the target to the hidden **ShadySimDeals Holdings** household, and then pays the active household. Cancellation or startup failure transfers nobody and pays nothing; if payment fails after transfer, the transfer is rolled back.
...
This release intentionally omits trait-aware seller moodlets, outcome-specific pregnant-Sim moodlets, ghosts, delayed outcomes, forced early multiple-birth detection, observer reaction buffs, sentiments, and persistent grudges. See [`SPECS_CHECKLIST.md`](SPECS_CHECKLIST.md) for detailed implementation status.
